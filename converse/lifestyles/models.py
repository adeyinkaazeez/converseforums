...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Lifestyle_Articles.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Lifestyle_Articles.Status.DRAFT)
    

class Lifestyle_Articles(models.Model): 
    User = get_user_model()
    ...
    likes = models.ManyToManyField(User, related_name='liked_lifestyle_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation') 
    section_name = models.CharField(max_length=50, default='Lifestyle', editable=False)
    ...
    def get_absolute_url(self):
        return reverse('lifestyles:post_detail',
    ...
    def get_comments(self):
        return self.lifestyle_comments.filter(lifestyle_parent=None).filter(active=True)
    
    
class Lifestyles_Comment(models.Model): 
    User = get_user_model()    
    post = models.ForeignKey(Lifestyle_Articles,                            
                             on_delete=models.CASCADE,                             
                             related_name='lifestyle_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='lifestyleuser_comments' )   
    ...
    lifestyle_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="lifestyle_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_lifestyle_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_lifestyle_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Lifestyle Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Lifestyles_Comment.objects.filter(lifestyle_parent=self).filter(active=True)
