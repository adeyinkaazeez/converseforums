...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Events.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Events.Status.DRAFT)
...
class Events(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_event_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Event', editable=False)  
    ...
    def get_comments(self):
        return self.event_comments.filter(event_parent=None).filter(active=True)
    
class Events_Comment(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Events,                            
                             on_delete=models.CASCADE,                             
                             related_name='event_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='eventuser_comments' )    
    ...
    event_parent = models.ForeignKey('self', 
                               on_delete=models.CASCADE, related_name="event_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_event_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_event_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Event Comment', editable=False) 
    class Meta:
        verbose_name = 'Business_Economy_Comment'          
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Events_Comment.objects.filter(event_parent=self).filter(active=True)
