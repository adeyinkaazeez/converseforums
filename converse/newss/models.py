...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Political.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Political.Status.DRAFT)
...
class Political(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_news_post', blank=True)
    ...
    def get_absolute_url(self):
        return reverse('newss:post_detail',
    ...
    def get_comments(self):
        return self.politics_comments.filter(politics_parent=None).filter(active=True)
    


#Comment section

class Politics_Comments(models.Model):
    User = get_user_model()     
    post = models.ForeignKey(Political,                            
                             on_delete=models.CASCADE,                             
                             related_name='politics_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='politicsuser_comments' )    
    ...
    politics_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="politics_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_news_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_news_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Political Comment', editable=False)
    #section_name = models.CharField(max_length=50, default='Politics', editable=False)
    ...
    def get_comments(self):
        return Politics_Comments.objects.filter(politics_parent=self).filter(active=True)
