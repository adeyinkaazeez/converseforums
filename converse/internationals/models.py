...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Internationals.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Internationals.Status.DRAFT)
...
class Internationals(models.Model): 
    ...
    author = models.ForeignKey(User,                             
                                ...
                                related_name='international_posts' )
    body = RichTextField()
    ...
    caption_two = models.TextField(blank=True ) 
    caption_three = models.TextField(blank=True) 
    caption_four = models.TextField(blank=True ) 
    likes = models.ManyToManyField(User, related_name='liked_international_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='International News', editable=False)
    ...
    def get_absolute_url(self):
        return reverse('internationals:post_detail',
    ...
    def get_comments(self):
        return self.international_comments.filter(international_parent=None).filter(active=True)
    



class Internationals_Comment(models.Model):
    User = get_user_model()     
    post = models.ForeignKey(Internationals,                            
                             on_delete=models.CASCADE,                             
                             related_name='international_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='internationaluser_comments' )      
    ...
    international_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="inter_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_international_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_international_comment', blank=True)
    section_name = models.CharField(max_length=50, default='International News Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Internationals_Comment.objects.filter(international_parent=self).filter(active=True)
