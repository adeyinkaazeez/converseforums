...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Loves.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Loves.Status.DRAFT)
...
class Loves(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_love_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation') 
    section_name = models.CharField(max_length=50, default='Love & Relationship', editable=False)
    ...
    def get_absolute_url(self):
        return reverse('loves:post_detail',
                       args=[self.publish.year,                             
                              self.publish.month,                             
                              self.publish.day,                             
                              self.slug])
    def get_comments(self):
        return self.love_comments.filter(love_parent=None).filter(active=True)
    
    
class Love_Comments(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Loves,                            
                             on_delete=models.CASCADE,                             
                             related_name='love_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='loveuser_comments' )    
    ...
    love_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="love_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_love_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_love_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Love & Relationship Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Love_Comments.objects.filter(love_parent=self).filter(active=True)
