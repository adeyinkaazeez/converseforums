...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Howtos.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Howtos.Status.DRAFT)
...
class Howtos(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_howto_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='How-to', editable=False)
    ...
    def get_absolute_url(self):
        return reverse('howtos:post_detail',
    ...
    def get_comments(self):
        return self.howto_comments.filter(how_parent=None).filter(active=True)
    
class Howtos_Comment(models.Model):  
    User = get_user_model()  
    post = models.ForeignKey(Howtos,                            
                             on_delete=models.CASCADE,                             
                             related_name='howto_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='howtouser_comments' )     
    ...
    how_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="how_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_howto_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_howto_comment', blank=True)
    section_name = models.CharField(max_length=50, default='How-to Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Howtos_Comment.objects.filter(how_parent=self).filter(active=True)
