...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Entertainments.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Entertainments.Status.DRAFT)
...
class Entertainments(models.Model): 
    ...
    author = models.ForeignKey(User,                             
                                ...
                                related_name='entertainment_posts' )
    body = RichTextField()
    ...
    likes = models.ManyToManyField(User, related_name='liked_entertainment_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Entertainment', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('entertainments:post_detail',
    ...
    def get_comments(self):
        return self.entertainment_comments.filter(enter_parent=None).filter(active=True)


class Entertainments_Comment(models.Model):    
    User = get_user_model() 
    post = models.ForeignKey(Entertainments,                            
                             on_delete=models.CASCADE,                             
                             related_name='entertainment_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='entertainmentuser_comments' )      
    ...
    enter_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="enter_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_entertainment_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_entertainment_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Entertainment Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Entertainments_Comment.objects.filter(enter_parent=self).filter(active=True)
