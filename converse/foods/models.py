...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Foods.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Foods.Status.DRAFT)
...
class Foods(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_food_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation') 
    section_name = models.CharField(max_length=50, default='Food', editable=False)  
    ...
    def get_absolute_url(self):
        return reverse('foods:post_detail',
    ...
    def get_comments(self):
        return self.food_comments.filter(food_parent=None).filter(active=True)
    
class Foods_Comment(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Foods,                            
                             on_delete=models.CASCADE,                             
                             related_name='food_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='fooduser_comments' )   
    ...
    food_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="food_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_food_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_food_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Food Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Foods_Comment.objects.filter(food_parent=self).filter(active=True)
