...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Happenings.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Happenings.Status.DRAFT)
...
class Happenings(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_happening_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Happening', editable=False)  
    ...
    def get_absolute_url(self):
        return reverse('happening:post_detail',
    ...
    def get_comments(self):
        return self.happenings_comments.filter(happening_parent=None).filter(active=True)
    
class Happenings_Comment(models.Model):    
    User = get_user_model() 
    post = models.ForeignKey(Happenings,                            
                             on_delete=models.CASCADE,                             
                             related_name='happenings_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='happeningsuser_comments' )     
    ...
    happening_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="happening_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_happening_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_happening_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Happening Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Happenings_Comment.objects.filter(happening_parent=self).filter(active=True)
